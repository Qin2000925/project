...
MODEL_DIR = 'e:/captcha/model/'
# 下载好谷歌训练好的Inception-v3模型名称
MODEL_FILE = 'tensorflow_inception_graph.pb'

# 由于训练数据会被使用多次，所以将原始图片通过inception-v3计算得到的特征
# 向量保存在文件中，免去重复计算，下面为地址
CACHE_DIR = 'd:/12306/inception_content/tmp_tensor'

# 训练图片的路径
INPUT_DATA_DIR = 'd:/12306/inception_content/content'

# 验证和测试数据的百分比
VALIDATAION_PERCENTAGE = 0
TEST_PERCENTAGE = 0
...
def main():
    ...
    with tf.name_scope('result'):
        weights = tf.Variable(initial_value=tf.truncated_normal(
            shape=[BOTTLENECK_TENSOR_SIZE, n_class], stddev=0.01
        ))
        biases = tf.Variable(initial_value=tf.zeros(shape=[n_class]))
    ...
    with tf.Session() as sess:
        ...
        saver.save(sess=sess, save_path='D:/12306/inception_content/save/model.ckpt')

...
class InsertTicket(object):
    def __init__(self, seat=SEAT_TYPE_CODE, date=TRAIN_DATE, from_station=FROM_STATION, to_station=TO_STATION, train=TRAINS_NO):
        self.headers = {
            'Accept-Encoding': 'gzip, deflate, br',
            'Accept-Language': 'zh-CN,zh;q=0.9',
    ...
    def get_ticket(self):
        print('开始查余票......')
        self.station_dict[self.from_station] = cons.city_code(self.from_station)
        self.station_dict[self.to_station] = cons.city_code(self.to_station)
        from_station = self.station_dict[self.from_station]
        to_station = self.station_dict[self.to_station]
    ...
    def post_data(self, secretStr, info):
        ...
        check_order_info_data = {
            ...
            'passengerTicketStr': '1,0,1,晏军,1,%s,18368142790,N'% PASSENGERS_ID[0],
            'oldPassengerStr': '晏军,1,%s,1_'% PASSENGERS_ID[0],
            'tour_flag': TOUR_FLAG,
            'randCode': '',
            'whatsSelect': '1',
            '_json_att': '',
